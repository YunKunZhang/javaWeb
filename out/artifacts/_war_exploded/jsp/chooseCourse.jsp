            ...
            <div class="ui_content">
                <div class="ui_tb">
                    <table class="table" id="elect" cellspacing="0" cellpadding="0" width="100%" align="center"
                        ...
                        <tbody>
                        <c:if test="${identity!='teacher'}">
                            <c:forEach items="${pageContext.request.getAttribute('info')}" var="temp" begin="0"
                                       step="1">
                                <tr>
                                    <td><c:out value="${temp.num}"/></td>
                                    <td><c:out value="${temp.name}"/></td>
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        ...
        ifTakeCourse = $('.numberSum').html() > 0 ? true : false;
        let optionSum = $('.numberSum').html();//可选课程总数
        let nowPage = 1;//当前页码
        let status;//课程状态
        ...
        function ajaxPaging(pageNum) {
            $.ajax({
                ...
                url: "paging",
                async: true,
                data: {"pageNum": pageNum, "pageName": "optional"},
